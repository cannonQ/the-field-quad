...
import {unspentBoxesForBigInt, unspentBoxesForV1} from "./explorer";
...
export async function get_utxos(
  ...
  const unspent = await unspentBoxesForBigInt(walletAddr);
  const finalUtxos = [];
  let assets = [];
  let addBox = false;
  // Convert to BigInt
  let ergAmtRemainder = BigInt(reqErg);
  let tokenAmtRemainder = BigInt(reqtokenAmt);
  ...
  unspent?.sort((a, b) => {
    ...
    if (aValue > bValue) return -1; // If a's value is greater, sort a before b
    if (aValue < bValue) return 1;  // If b's value is greater, sort b before a
    return 0;                       // If they are equal, no sorting order change
  });

  for (let i = 0; i < unspent.length; i++) {
    let box = unspent[i];
    // Convert box.value to BigInt for comparison and subtraction
    let boxValue = BigInt(box.value);

    if (ergAmtRemainder > 0) {
      addBox = true;
      ergAmtRemainder -= boxValue;
    }

    if (tokenId !== "") {
      for (let j = 0; j < box.assets.length; j++) {
        let tok = box.assets[j];
        // Convert tok.amount to BigInt for comparison and subtraction
        let tokAmount = BigInt(tok.amount);

        if (tok.tokenId === tokenId && tokenAmtRemainder > 0) {
          addBox = true;
          tokenAmtRemainder -= tokAmount;
          break;
        }
      }
    }
    ...
    if (addBox) {
      for (let k = 0; k < box.assets.length; k++) {
        ...
        assets.push({
          tokenId: a.tokenId,
          // Keep amount as string to preserve precision when passed out of the function
          amount: BigInt(a.amount).toString(),
        });
      }
      finalUtxos.push({
        boxId: box.boxId,
        value: box.value.toString(), // Convert to string to preserve precision
...
export async function getUtxosForSelectedInputs(
  inputAddressList,
  ergAmount,
  tokens = [],
  tokensAmountToSend = []
  ...
  try {
    utxos = parseUtxos(
      await Promise.all(
        inputAddressList.map(async (address) => {
  ...
  var selectedUtxos = [],
    unSelectedUtxos = utxos,
    i = 0;
  while (
    !hasEnoughSelectedTokens(selectedUtxos, tokens, tokensAmountToSend) &&
    i < 1000
  ) {
    ...
    for (const j in tokens) {
      if (tokensAmountToSend[j] > 0 && !boxFound) {
        ...
        boxIndex = unSelectedUtxos.findIndex((utxo) =>
          utxo.assets.map((tok) => tok.tokenId).includes(tokens[j].tokenId)
        );
  ...
  while (
    BigInt(Math.round(ergAmount * NANOERG_TO_ERG)) >
      getUtxosListValue(selectedUtxos) &&
    unSelectedUtxos.length > 0
  ) {
...
export function hasEnoughSelectedTokens(
  utxos,
  requiredTokens,
  requiredTokenAmounts
) {
  const utxosTokens = getTokenListFromUtxos(utxos);
  const fixedRequiredTokenAmounts = requiredTokenAmounts.map((amount, id) =>
    Math.round(
      parseFloat(amount.toString()) * Math.pow(10, requiredTokens[id].decimals)
    )
  );
  ...
  for (const i in requiredTokens) {
    if (fixedRequiredTokenAmounts[i] > 0) {
      ...
      for (const tokenId of Object.keys(utxosTokens)) {
        ...
        if (
          tokenId === requiredTokens[i].tokenId &&
          utxosTokens[tokenId] < fixedRequiredTokenAmounts[i]
        ) {
