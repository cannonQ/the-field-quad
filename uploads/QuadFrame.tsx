...
export function QuadFrame({ children, compact = false }: QuadFrameProps) {
  return (
    <div className={`relative ${compact ? 'pt-8 pb-2 px-1' : 'pt-10 pb-4 px-2'} w-full ${compact ? 'max-w-[400px]' : 'max-w-[600px]'} mx-auto`}>
      ...
      {/* This sits behind the main frame visually but z-index above to hide the top border segment if needed, 
          or we design it to look connected.
          Mockup shows a dark gray handle bar connected by two colored circles.
      */}
      ...
      {/* Connection Circles */}
      <div className={`absolute ${compact ? 'top-6' : 'top-8'} left-1/2 -translate-x-1/2 ${compact ? 'w-[calc(100%+4px)]' : 'w-[calc(100%+8px)]'} max-w-${compact ? '[404px]' : '[608px]'} flex justify-between ${compact ? 'px-8' : 'px-12'} z-20 pointer-events-none`}>
      ...
      <div className="relative z-10 rounded-[2.5rem] p-[4px] shadow-2xl overflow-hidden"
           style={{
      ...
      >
        ...
        <div className="bg-[#0f1115] rounded-[2.3rem] p-4 h-full relative overflow-hidden">
            {/* Subtle Texture/Noise if desired */}
            ...
            <div className="relative z-10">
             ...
             <div className="mt-4 mb-1 text-center">
                <p className={`${compact ? 'text-[8px]' : 'text-[10px]'} text-gray-500 font-mono tracking-wider`}>*The Field includes: UNC, Arizona, Tennessee + 60 others</p>
            </div>
        </div>
      </div>
    </div>
