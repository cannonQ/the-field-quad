...
export function QuadCard({ option, index, color, poolTotal, onSelect, compact = false }: QuadCardProps) {
  ...
  return (
    <div className={cn(
      ...
      `rounded-${compact ? 'xl' : '2xl'} p-${compact ? '3' : '6'} min-h-${compact ? '[140px]' : '[220px]'}`,
      colorStyles[color]
    )}>
      ...
      <div className={`flex items-center gap-1 mb-2 ${compact ? 'flex-col' : ''}`}>
        ...
        {isField && (
           <img src={shieldWhite} alt="Shield" className={`${compact ? 'w-4 h-4' : 'w-6 h-6'} object-contain opacity-90`} />
        )}
      </div>

      <div className={`space-y-1 mb-${compact ? '3' : '6'}`}>
